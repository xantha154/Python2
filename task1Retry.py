...
class Vehicle:
    ...
    def __init__(self, brand: str, model: str, year: int) -> None:
        ...
        Атрибуты публичные так как они являются базовыми характеристиками которые чаще всего используются при взаимодействии напрямую с обьектом
...
class Car(Vehicle):
    ...
    def __init__(self, brand: str, model: str, year: int, passenger_capacity: int) -> None:
        ...
        Наследуются специфические методы которые есть только у легковых автомобилей таких как вместимость пассажиров
...
class Truck(Vehicle):
    ...
    def __init__(self, brand: str, model: str, year: int, max_payload: float) -> None:
        ...
        max_pauload непубличный так как не должен изменяться без проверок
        :param brand: Бренд автомобиля.
        :param model: Модель автомобиля.
        :param year: Год выпуска автомобиля.
